...
class doc3dcrLoader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        im_path = pjoin(self.root, 'img_dewarp1',  im_name + '.png')  
        # gt_path = pjoin(self.root, 'img_dewarp2',  im_name + '.png')  
        bm_path = pjoin(self.root, 'bm_npy1' , im_name + '.npy')
        bm_path_origin = pjoin(self.root, 'bm_npy' , im_name + '.npy')
        ...
        # gt_resize = cv2.resize(gt,self.img_size)
        ...
        base = (np.arange(self.size)/self.size-0.5)*2
        base_x = np.expand_dims(base,0).repeat(self.size,axis=0)
        base_y = np.expand_dims(base,1).repeat(self.size,axis=1)
        base_coordinate = np.concatenate((base_x.reshape(self.size,self.size,1),base_y.reshape(self.size,self.size,1)),axis=-1)
        base_coordinate = torch.from_numpy(base_coordinate)
        # prepare bm
        ...
        bm_origin = np.load(bm_path_origin) # -> ndarray (448*448*2) 0~448 float
        bm_tmp_origin = bm_origin.copy()
        
        # prepare unwarp image
        bm0_tmp = bm_tmp[:,:,0].astype(np.float32)
        bm1_tmp = bm_tmp[:,:,1].astype(np.float32)
        bm0_tmp_origin = bm_tmp_origin[:,:,0].astype(np.float32)
        bm1_tmp_origin = bm_tmp_origin[:,:,1].astype(np.float32)
        uw_pdf = cv2.remap(pdf.astype(np.uint8),bm0_tmp_origin,bm1_tmp_origin,cv2.INTER_CUBIC)
        uw_im = cv2.remap(im.astype(np.uint8),bm0_tmp,bm1_tmp,cv2.INTER_CUBIC)
        uw_im_resize = cv2.resize(uw_im,self.img_size)
        # uw_im = cv2.imread(im_path.replace('img_dewarp1','img_dewarp2'))
        ...
        if 1:
            ...
            cv2.imshow('um',uw_im_resize)
            cv2.imshow('pdf',uw_pdf_resize)
...
#            im,depth,msk=data_aug(im,depth,bg)
        ...
        uw_pdf = self.transform(uw_pdf_resize).float()
        content_mask = self.transform(content_mask_resize).float()
        base_coordinate = base_coordinate.permute(2,0,1).float()
        bm = torch.from_numpy(bm_resize).permute(2,0,1).float()
        
        # if self.is_transform:
        #     im_org, im, depth_org,depth, mask, edge, bm_org, bm,content_mask_org = self.transform(im, depth, mask, edge, bm,content_mask)
        return im, bm, content_mask, uw_im, base_coordinate
