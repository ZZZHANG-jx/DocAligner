...
if __name__ == '__main__':
    ...
    parser.add_argument("--img_path", type=str, default= '/media/jiaxin/learning_data/dewarp/synth/crop_data/test/patch/magazine-014-118-110_warp69521curve_0703.png')
    parser.add_argument("--flow_path", type=str, default= '/media/jiaxin/learning_data/dewarp/synth/crop_data/test/patch_flow/magazine-014-118-110_warp69521curve_0703.npy')
    ...
    flow[0,:,:] = temp1
    flow[1,:,:] = temp0
    # print(distortedImg.shape,flow.shape)
    resImg, resMsk = rectification(distortedImg, flow)
    resImg_enhance = resImg.copy()
    for i in range(resImg_enhance.shape[0]):
        for j in range(resImg_enhance.shape[1]):
            if np.sum(resImg_enhance[i,j]) == np.sum(np.array((255,255,255))):
                resImg_enhance[i,j] = resImg_enhance[i-1,j-1]
    io.imsave('./result.png', resImg) 
    io.imsave('./result_enhance.png', resImg_enhance) 
