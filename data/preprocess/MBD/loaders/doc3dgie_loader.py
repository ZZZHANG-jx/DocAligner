...
class doc3dgieLoader(data.Dataset):
    """
    ...
    def __getitem__(self, index):
        ...
        mask = cv2.imread(pjoin(self.root, 'mask', im_name + '.png'))[:,:,0]
        ...
        edge = cv2.dilate(edge, kernel=kernel,iterations=3)
        edge_resize = cv2.resize(edge, self.img_size)
        edge_resize[edge_resize > 10] = 255
        edge_resize[edge_resize <= 10] = 0  
        ...
        if 'train' in self.split or 'debug' in self.split:
            ...
            if random.uniform(0,1) > 0:
                ...
                im_resize,mask_resize,edge_resize = self.random_back_ground(back_ground,im_resize,cv2.erode(mask_resize,np.ones((3,3)),iterations=1),edge_resize)
                # im_resize,mask_resize,edge_resize = self.random_back_ground(back_ground,im_resize,mask_resize,edge_resize)
            im_blur = cv2.GaussianBlur(im_resize,(15,15),0,0)
            # im_resize[cv2.dilate(edge_resize, kernel=np.ones((5,5)))==255] = im_blur[cv2.dilate(edge_resize, kernel=np.ones((5,5)))==255]
            # im_resize[edge_resize==255] = im_blur[edge_resize==255]
            ### erase augment
            im_resize = im_blur
        ...
        mask_resize[mask_resize > 10] = 255
        mask_resize[mask_resize <= 10] = 0  
...
#            im,depth,msk=data_aug(im,depth,bg)
        ...
        if 1:
            ...
            cv2.imshow('im_hsv',cv2.cvtColor(im_resize,cv2.COLOR_RGB2HSV))
            cv2.imshow('mask',mask_resize)
            cv2.imshow('edge',edge_resize)
            cv2.waitKey(0)


        im = self.transform_hsv(im_resize).float()
    ...
    def transform_hsv(self,img):
        ...
        img = img.astype(np.float)/np.asarray((180,255,255))
        img = img.transpose(2, 0, 1)
        img = torch.from_numpy(img)
        return img
