...
from models.glu_doc.feature_backbones.ResNet_features import ResNetPyramid
...
class GLUNet_model(nn.Module):
    ...
    def __init__(self, evaluation, div=1.0, iterative_refinement=False,
        ...
        self.deconv_half = BilinearConvTranspose2d(2, 2, kernel_size=4, stride=2, padding=1)
        self.deconv_full = BilinearConvTranspose2d(2, 2, kernel_size=4, stride=2, padding=1)

        # self.out_half = nn.Sequential(
        #     nn.Conv2d(258,128,kernel_size=3,padding=1,stride=1,dilation=1),
        #     nn.BatchNorm2d(128),
        #     nn.ReLU(inplace=True),
        #     nn.Conv2d(128,64,kernel_size=3,padding=1,stride=1,dilation=1),
        #     nn.BatchNorm2d(64),
        #     nn.ReLU(inplace=True),
        #     nn.Conv2d(64,2,kernel_size=1,padding=0,stride=1,dilation=1)
        # )
        self.out_half = nn.Sequential(
            nn.Conv2d(130,64,kernel_size=3,padding=3,stride=1,dilation=3),
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
            nn.Conv2d(64,32,kernel_size=3,padding=3,stride=1,dilation=3),
            nn.BatchNorm2d(32),
            nn.ReLU(inplace=True),
            nn.Conv2d(32,2,kernel_size=1,padding=0,stride=1,dilation=1)
        )
        self.out_full = nn.Sequential(
            nn.Conv2d(130,64,kernel_size=3,padding=3,stride=1,dilation=3),
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
            nn.Conv2d(64,32,kernel_size=3,padding=3,stride=1,dilation=3),
    ...
    def forward(self, im_target, im_source, im_target_256, im_source_256):
        ...
        # # level 1/2 of original resolution
        # ratio = 1.0 / 2.0        
        # up_flow1 = self.deconv_half(flow1)
        # warp_c2_half = warp(c2_half, up_flow1*div*ratio)
        # flow_half = self.out_half(torch.cat((c1_half,warp_c2_half,up_flow1),dim=1))+up_flow1#c=130

        # # level 1/1 of original resolution
        # ratio = 1.0 / 1.0        
        # up_flow_half = self.deconv_full(flow_half)
        # warp_c2_full = warp(c2_full, up_flow_half*div*ratio)
        # flow_full = self.out_full(torch.cat((c1_full,warp_c2_full,up_flow_half),dim=1))+up_flow_half#c=130
